...
Note: for the examples above, use the following:
- For the single site selection example (Site ID 3586). This site has a single analysis unit.
- For the multiple site selection example (Site IDs 5120,5700,16549). Each site has multiple analysis units.
...
names <- names[sapply(collids,function(x) which(grepl(paste0("^",x,"$"),names$collid))),]
...
names2 <- names2[sapply(collids,function(x) which(grepl(paste0("^",x,"$"),names2$collid))),]
...
#For the example collection from above, Crystal Ball Cave, siteID 3586:
