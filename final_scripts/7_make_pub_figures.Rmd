...
required.packages <- c("leaflet","maps","naturalearth","sf","terra","ggplot2","dplyr","neotoma2","reshape","rnaturalearth","viridis","jsonlite","httr","rlist","cowplot")
checkAndInstall(required.packages)
```

```{r refresh data}
rmlist <- c("alldates","geochrons","newchron.range","newchron.all","found","corrected")
rm(list = subset(rmlist, sapply(rmlist, exists)))
```
...
```{r Figure 2b and Figure S2 objects}
...
```{r Figure 2b and Figure S2 shapefile}
...
datecounts <- subset(datecounts,apply(datecounts[,2:5],1,sum) != 0)
datecounts <- datecounts[order(apply(datecounts[,2:5],1,sum),decreasing = T),]
temp1 <- reshape::melt(datecounts);colnames(temp1) <- c("Ecoregion","Interval","Proportion")

alldates.vect <- sapply(split(dates,dates$interval),
                     function(x) vect(x[,c("long","lat")],geom = c("long","lat"),crs = crdref))
temp <- sapply(alldates.vect,function(x) relate(vect(eco1), x, "contains"))
alldatecount <- sapply(temp, function(x) apply(x,1,sum))
temp <- data.frame(eco1$NA_L1NAME,alldatecount)
colnames(temp) <- c("Ecoregion","LH","EH","DG","LGM")
temp <- do.call("rbind",by(temp,temp$Ecoregion,function(x) apply(x[,2:5],2,function(x) sum(x))))
alldatecounts <- data.frame(Ecoregion = rownames(temp), temp[,4:1])
alldatecounts$Ecoregion <- gsub("NORTHWESTERN","NW",alldatecounts$Ecoregion)
alldatecounts$Ecoregion <- reorder(alldatecounts$Ecoregion,
                                apply(alldatecounts[,2:5],1,sum),decreasing = T)
alldatecounts <- subset(alldatecounts,apply(alldatecounts[,2:5],1,sum) != 0)
alldatecounts <- alldatecounts[order(apply(alldatecounts[,2:5],1,sum),decreasing = T),]
temp2 <- reshape::melt(alldatecounts);colnames(temp2) <- c("Ecoregion","Interval","Proportion")

figureS2a <- ggplot(temp2,aes(x = Interval,y = Proportion,fill = Ecoregion)) + geom_col(position = "fill") + scale_fill_brewer(palette = "Paired") + ggtitle("All dates") + theme(legend.position="none")
figureS2b <- ggplot(temp1,aes(x = Interval,y = Proportion,fill = Ecoregion)) + geom_col(position = "fill") + scale_fill_brewer(palette = "Paired") + ggtitle("New dates only") 
figureS2legend <- get_legend(figureS2b)
figureS2b <- ggplot(temp1,aes(x = Interval,y = Proportion,fill = Ecoregion)) + geom_col(position = "fill") + scale_fill_brewer(palette = "Paired") + ggtitle("New dates only") + theme(legend.position="none") 

ggdraw(plot_grid(figureS2a, figureS2b, figureS2legend, nrow = 1, rel_widths = c(1,1,0.6)))
...
```{r join old chronologies to old sample ages}
...
while (I <= length(sampleids)) {
  ...
  I <- i + 1; J <- min(c(length(sampleids),(i + 100)))
}
...
## Report values in Table 3/Figure 6a
```{r}
Table3 <- cbind(
...
print(table(clean_dates$agetypeid))
print(table(clean_dates$geochrontypeid))
```

## Technical validation section
```{r dates}
newdates <- read.csv(here("outputs/newdates-uploaded.csv"))
newdate_counts <- apply(newdates[,c("ndb.geochronology.geochronid","ndb.collectionunits.collectionunitid","ndb.sites.siteid")],2,function(x) length(unique(x)))
print(newdate_counts)
```
...
sampleage_diffs <- sapply(compare_sampleages, function(x) {
  return(data.frame(
...
print(sampleage_diffs)
chron_diffs <- sapply(compare_chrons, function(x) {
  return(data.frame(
    old.agediff = median(x$old.agediff, na.rm=T),
    new.agediff = median(subset(x$new.agediff, !is.na(x$old.agediff)), na.rm=T),
    difference = median(x$new.agediff - x$old.agediff, na.rm=T))
...
table2 <- vector(mode = "list",length = 3); names(table2) <- c("a","b","c")
table2$a <- data.frame(updateids$chrons[,c(2,3,4,5,6,1)])
colnames(table2$a) <- c("collectionunitid","agemodel","ageboundolder","ageboundyounger",
                       "dateprepared","chronologyid")
table2$a$agemodel <- c("Bayesian unit bounds","Bayesian event distribution")

#get chroncontrol ids
table2$c <- data.frame(updateids$controls[,c(1,3,4,5,6,7,8)])
colnames(table2$c) <- c("chronid","analysisunitid","geochronid","labnumber","age","agelimitolder","agelimityounger")
geochronids <- vector(mode = "list", length = length(unique(table2$c$chronid)))
names(geochronids) <- unique(table2$c$chronid)
for (i in 1:length(unique(corr$chronid))) {
  temp <- get_from_tilia(values = unique(corr$chronid)[i], params = "chronologyid", 
                 meth = "getchronocontrolsbychronologyid")$data
  temp$chronid <- unique(corr$chronid)[i]
  geochronids[[i]] <- distinct(inner_join(
            temp[,c("chronid","chroncontrolid","analysisunitid","age","agelimitolder","agelimityounger")],
            table2$c[,c("chronid","analysisunitid","geochronid","age","agelimitolder","agelimityounger")]))
}
geochronids <- list.stack(geochronids)
temp <- distinct(inner_join(table2$c,geochronids))
table2$c <- temp[,c("chronid","analysisunitid","geochronid","labnumber",
                    "age","agelimitolder","agelimityounger","chroncontrolid")]

#get sample ids
samp_by_chron <- vector(mode = "list", length = length(unique(geochronids$chronid)))
for (i in 1:length(unique(geochronids$chronid))){
  temp <- get_from_tilia(values = unique(geochronids$chronid)[i], 
                          params = "chronologyid", meth = "getsampleagesbychronid")$data
  temp$chronologyid <- unique(geochronids$chronid)[i]
  samp_by_chron[[i]] <- temp
}; samp_by_chron <- distinct(list.stack(samp_by_chron))
found_sampleids <- unique(c(samp_by_chron$sampleid,samp_by_geochron$sampleid))
auid_by_samp <- vector(mode = "list", length = length(found_sampleids))
for (i in 1:length(found_sampleids)){
  if (!is.na(found_sampleids[i])){
    temp <- get_from_tilia(values = found_sampleids[i], 
                          params = "sampleid", 
                          meth = "getsampleparents")$data
    temp$sampleid <- found_sampleids[i]
    auid_by_samp[[i]] <- temp
  } 
}; auid_by_samp <- distinct(list.stack(auid_by_samp))

temp <- distinct(full_join(auid_by_samp[,c("sampleid","analysisunitid")], samp_by_chron))
table2$b <- data.frame(updateids$samples[,c(1,3,4,5,6,7)])
colnames(table2$b) <- c("chronologyid","analysisunitid","agemodel","age","ageolder","ageyounger")
table2$b <- distinct(full_join(table2$b,temp))
table2$b <- table2$b[order(table2$b$analysisunitid),]
table2$b <- table2$b[order(table2$b$chronologyid),]

View(table2$a);View(table2$b);View(table2$c)
```
